...
public class BiomesDataGenerator implements IDataGenerator {
    ...
    public static JsonObject generateBiomeInfo(Registry<Biome> registry, Biome biome) {
        ...
        // In 26.3, sky color is a Vector3fc, not a packed Integer
        final EnvironmentAttributeMap.Entry<Vector3fc, ?> skyColorEntry = biome.getAttributes().get(EnvironmentAttributes.SKY_COLOR);
        int skyColor = 0;
        if (skyColorEntry != null && skyColorEntry.argument() instanceof Vector3fc vec) {
            skyColor = ARGB.color(
                    ARGB.as8BitChannel(vec.x()),
                    ARGB.as8BitChannel(vec.y()),
                    ARGB.as8BitChannel(vec.z())
            ) & 0xFFFFFF;
        }
