...
public class RecipeDataGenerator implements IDataGenerator {
    ...
    public JsonElement generateDataJson() {
        ...
        for (RecipeEntry<?> recipeE : Objects.requireNonNull(DGU.getWorld()).getServer().getRecipeManager().values()) {
            ...
            if (recipe instanceof ShapedRecipe sr) {
                ...
                if (sl.craft(CraftingRecipeInput.EMPTY, registryManager).isEmpty()) continue;
                ...
                for (Ingredient ingredient : sl.getIngredientPlacement().getIngredients()) {
                    ...
                    ingredients.add(getRawIdFor(ingredient.getMatchingItems().toList().getFirst().value()));
                }
    ...
    private void generateShapedRecipe(DynamicRegistryManager registryManager, JsonObject finalObj, ShapedRecipe sr, int n) {
        if (sr.craft(CraftingRecipeInput.EMPTY, registryManager).isEmpty()) return; // see the shapeless case
        ...
        for (int i = 0; i < 9; i++) {
            ...
            var matching = stacks.get().getMatchingItems().toList(); // FIXME: fix when there are more than one matching stack
            if (matching.isEmpty()) {
                ingr.add(null);
                continue;
