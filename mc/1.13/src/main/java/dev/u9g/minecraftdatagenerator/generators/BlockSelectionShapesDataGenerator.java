...
public class BlockSelectionShapesDataGenerator implements IDataGenerator {
    // Unlike the existing collision helper, an outline query also needs a fluid state.
    private enum EmptyBlockView implements net.minecraft.world.BlockView {
        INSTANCE;
        public net.minecraft.block.entity.BlockEntity getBlockEntity(BlockPos pos) { return null; }
        public BlockState getBlockState(BlockPos pos) { return net.minecraft.block.Blocks.AIR.getDefaultState(); }
        public net.minecraft.fluid.FluidState getFluidState(BlockPos pos) { return getBlockState(pos).getFluidState(); }
        public int getMaxLightLevel() { return 15; }
    }
    @Override
    ...
    public JsonObject generateDataJson() {
        ...
        for (Block block : (Iterable<Block>) registry) {
